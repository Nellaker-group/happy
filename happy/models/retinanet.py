"""
Code adapted from: https://github.com/yhenon/pytorch-retinanet
"""
...
from happy.models.utils import losses
from happy.models.utils.anchors import Anchors
from happy.models.utils.retinanet_utils import (
    BasicBlock,
    Bottleneck,
    BBoxTransform,
    ClipBoxes,
    ...
    def forward(self, inputs, device):
        ...
        if self.training:
            return self.focalLoss(
                classification, regression, anchors, annotations, device
            )
        else:
            ...
            # NOTE: nms IoU threshold was changed to 0.4 from 0.5 here
            anchors_nms_idx = torchvision.ops.nms(
                transformed_anchors[0], scores[0, :, 0], 0.4
            )
...
def build_retina_net(num_classes, device, pretrained=False, resnet_depth=101):
    ...
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url(model_urls[f"resnet{resnet_depth}"], model_dir="."),
            strict=False,
        )
    return model
